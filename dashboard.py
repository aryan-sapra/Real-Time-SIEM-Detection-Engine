...
def refresh(_):
    ...
    high_alerts = len(alerts_df[alerts_df["severity"].isin(["high", "critical"])]) if not alerts_df.empty and "severity" in alerts_df.columns else 0
    critical_alerts = len(alerts_df[alerts_df["severity"] == "critical"]) if not alerts_df.empty and "severity" in alerts_df.columns else 0
    ...
    kpis = [
        ...
        kpi_card("Unique source IPs", unique_ips, C["accent"]),
        kpi_card("High / critical", high_alerts, C["yellow"] if high_alerts else C["text"],
                 sub=f"{critical_alerts} critical"),
    ...
    if alerts_df.empty:
        ...
        for _, row in alerts_df.iterrows():
            ...
            sev_color = {"critical": C["red"], "high": C["orange"], "medium": C["yellow"], "low": C["muted"]}.get(severity, C["muted"])
            mitre = row.get("mitre_id", "")
            src_ip = row.get("src_ip", "")
            alert_items.append(html.Div([
                html.Span(ts, style={"color": C["muted"], "marginRight": "8px", "minWidth": "65px", "display": "inline-block"}),
                html.Span(severity.upper(), style={"color": sev_color, "fontWeight": "700", "marginRight": "8px"}),
                html.Span(row["alert_type"].replace("_", " "), style={"color": C["text"], "marginRight": "8px"}),
                html.Span(f"{src_ip} ", style={"color": C["accent"], "marginRight": "8px"}) if src_ip else html.Span(),
                html.Span(f"{mitre} " if mitre else "", style={"color": C["purple"], "marginRight": "6px"}),
                html.Span(row["detail"], style={"color": C["muted"]}),
            ], className="alert-row", style={"padding": "5px 0", "borderBottom": f"1px solid {C['border']}"}))
    ...
    if df.empty:
        ...
        for _, row in recent.iterrows():
            ...
            log_rows.append(html.Div([
                ...
                html.Span(f"[{tag}]", style={"color": color, "marginRight": "8px",
                                              "minWidth": "145px", "display": "inline-block"}),
                html.Span(
                    f"{row.get('src_ip', '')} " if row.get("src_ip", "") else "",
                    style={"color": C["accent"], "marginRight": "8px"}
                ),
                html.Span(
                    f"user={row.get('username', '')} " if row.get("username", "") else "",
                    style={"color": C["purple"], "marginRight": "8px"}
                ),
